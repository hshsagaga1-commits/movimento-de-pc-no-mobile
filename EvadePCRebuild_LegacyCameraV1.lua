-- Evade PC Rebuild - Legacy Camera V1
-- From-scratch native Legacy mouse-lock state owner.
-- Does not load old camera scripts and does not write Camera.CFrame.
...
local VERSION = "EvadePCRebuild-LegacyCamera-V1"
local LEGACY_PLACE_ID = 96537472072550
local BIND_NAME = "__EvadePCRebuildLegacyCameraV1"
local SHOULDER_OFFSET = Vector3.new(2, 0.5, 0)

local previousCleanup = ENV.__EvadePCRebuildLegacyCameraV1Cleanup
...
if game.PlaceId ~= LEGACY_PLACE_ID then
    ...
    ENV.EvadePCRebuildLegacyCameraV1 = api
    return api
end
...
ENV.__EvadePCRebuildLegacyCameraV1Cleanup = function()
    enabled = false
    ...
    ENV.EvadePCRebuildLegacyCameraV1 = nil
    ENV.__EvadePCRebuildLegacyCameraV1Cleanup = nil
end
