...
local VERSION="EvadePC-Joystick-V5-balanced-sectors-legacy-route"
local BIND_NAME="__EvadePCJoystickV5"
local GUI_NAME="EvadePCJoystickV5Gui"
...
-- Dry diagonal swap remains active across the whole W center band.
-- Recenter/release or leaving the W/diagonal upper arc clears it.
...
for _,name in ipairs({"EvadePCJoystickV5Gui","EvadePCJoystickV4Gui","EvadePCJoystickV3Gui"}) do
...
local function classifyDigital(v)
    ...
    local raw,rawName,angle=rawClassify(v)
    lastRawChord=rawName
    local absAngle=math.abs(angle)

    -- Pure A/D or S means the finger left the upper W/diagonal sweep.
    if absAngle>DIAG_END_DEG then
        diagonalLatch=nil
        return raw
    end

    -- A diagonal establishes which side is latched.
    if rawName=="W+A" then
    ...
    -- If the same held touch crosses the W sector, keep the previous diagonal
    -- instead of generating W. On entering the opposite diagonal sector, the
    -- side changes in that same render step: WD -> WA or WA -> WD.
    if rawName=="W" and diagonalLatch=="D" then
        return {W=true,D=true}
    elseif rawName=="W" and diagonalLatch=="A" then
...
ENV.EvadePCJoystickV5=api

ENV.__EvadePCJoystickV5Cleanup=function()
    enabled=false
    jumpToken+=1
    releaseMovement()
    ...
    ENV.EvadePCJoystickV5=nil
    ENV.__EvadePCJoystickV5Cleanup=nil
end
