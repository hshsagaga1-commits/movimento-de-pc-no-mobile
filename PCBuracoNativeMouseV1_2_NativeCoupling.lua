-- PCBuracoNativeMouseV1_2_NativeCoupling.lua
...
local VERSION = "PC-BURACO-NATIVE-MOUSE-V1.2-NATIVE-COUPLING"
...
-- V1.2: restore the native PC mouse-lock/character coupling around the same
-- V1.1 camera path. The helper owns only native lock state + Evade CamStats
-- mouse identity. It never writes Camera.CFrame, Focus, RootPart or Head CFrame.
do
    local url = "https://raw.githubusercontent.com/hshsagaga1-commits/movimento-de-pc-no-mobile/044f6797ad5777bffd3f2a4fd1e30ff33d2e4e71/PCNativeBuracoCouplingV1.lua?_cb="
        .. HttpService:GenerateGUID(false)
    local source = game:HttpGet(url, true)
    local chunk, err = loadstring(source)
    if not chunk then error(err) end
...
local nativeCouplingCleanup = ENV.__PCNativeBuracoCouplingV1Cleanup
...
ENV.PCBuracoNativeMouseV11 = {
    ...
    GetState = function()
        return {
            ...
            forcesMouseBehavior = true,
            forcesRotationType = true,
            forcesMouseLockOffset = true,
            nativeCoupling = type(ENV.PCNativeBuracoCouplingV1) == "table",
            nativeCouplingState = type(ENV.PCNativeBuracoCouplingV1) == "table"
                and ENV.PCNativeBuracoCouplingV1.GetState() or nil,
            smoothing = false,
        }
    end,
}
...
ENV.__PCBuracoNativeMouseV11Cleanup = function()
    ...
    if type(nativeCouplingCleanup) == "function" then
        pcall(nativeCouplingCleanup)
    end
