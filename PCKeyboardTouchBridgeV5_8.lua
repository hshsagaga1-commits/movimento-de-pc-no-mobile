...
-- from 55 ms to 60 ms. There is no added delay before Space-down: the existing
...
source,replacements=source:gsub(
    ...
    "local SPACE_PULSE_SECONDS=0.060",
    1
)

if replacements~=1 then
    error("V5.8 jump timing patch failed: expected exactly one 55 ms Space pulse constant")
end
...
if type(getgenv().PCKeyboardTouchBridgeV52)=="table" then
    getgenv().PCKeyboardTouchBridgeV52.Version="5.8-zero-delay-60ms-space-hold"
end
