...
-- from 55 ms to 200 ms. There is no added delay before Space-down: the existing
...
source,replacements=source:gsub(
    ...
    "local SPACE_PULSE_SECONDS=0.200",
    1
)
...
if type(getgenv().PCKeyboardTouchBridgeV52)=="table" then
    getgenv().PCKeyboardTouchBridgeV52.Version="5.9-zero-delay-200ms-space-hold"
end
