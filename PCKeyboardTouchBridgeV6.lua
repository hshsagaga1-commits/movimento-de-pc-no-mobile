...
-- V6.7 deliberately keeps the exact proven V5.9 jump path. Only joystick
-- ownership/latching is changed from the stable bridge.
...
local function replaceOncePlain(text,needle,replacement,label)
    ...
    if not first then error("V6.7 patch failed: missing "..label) end
    if text:find(needle,last+1,true) then error("V6.7 patch failed: duplicate "..label) end
    return text:sub(1,first-1)..replacement..text:sub(last+1)
end

-- Exact known-working V5.9 200 ms Space burst.
local oldJumpState=[[
...
-- Persistent side-sweep state must live outside refreshKeys.
...
local newRefresh=[[
    ...
    local function chooseDigitalChord(x,z)
        ...
        -- Forward: smaller W zone, broad outer WA/WD, pure A/D at the sides.
        if z<0 then
        ...
        -- Backward half stays exactly on the tested V5.9 split.
...
if type(getgenv().PCKeyboardTouchBridgeV52)=="table" then
    getgenv().PCKeyboardTouchBridgeV52.Version="6.7-v59-jump-2d-zones-dry-side-sweep"
end
