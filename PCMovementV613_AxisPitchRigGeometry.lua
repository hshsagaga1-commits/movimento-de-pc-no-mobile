...
local UI_NAME="PCMovementV613Panel"

--[[
    V613 / AXIS, PITCH, AND RIG GEOMETRY CONTROL

    V604-V612 and the post-V612 audit are accepted evidence. No search is
...
-- every touch. V613 reuses the already-validated V604 proof so this controlled
...
if proofInitCount~=1 or proofStatusCount~=1 then
    error("V613 refused: exact V604 persisted-proof initialization anchor missing")
end
...
getgenv().PCMovementVersion="V613-AxisPitchRigGeometry"
getgenv().PCInputBridgeMode="v613-axis-pitch-rig-geometry"
...
local MIN_BUCKET_SAMPLES=25          -- legacy V612 classifier only; V613 does not use it
...
local function startProbe()
    ...
    addEvidence("PROBE","V613 paired standing control started; choose explicit phase A then B")
    return true,"started"
end
...
local function _legacyV612Classification()
    ...
    local nextSuspect=causal=="not-supported"
        and "GetSubjectPosition subject origin -> CameraFocus/orbit reference (not investigated in V613)"
...
-- V613 classification deliberately separates descriptive effect, temporal
-- uncertainty, and causal wording. V612's effect-size gate above remains only
-- as historical code and is shadowed here; it is not used by V613 diagnostics.
...
local function classification()
    ...
    return {
        ...
        experimentEligible=false,
        astra6MaxJustified=firstExplanation=="unproved" and "unproved: evaluate V613 evidence first" or false,
    }
end

getgenv().PCV613Diagnostics=function()
    local base=baseSafe()
    ...
    local result={
        version="V613-AxisPitchRigGeometry",
        bridgeMode=getgenv().PCInputBridgeMode,
        probePurpose="axis-pitch-rig-geometry-touch-vs-v604-relay",
        ...
        validationReady=callbackErrors==0 and frameCorrelationErrors==0 and decision.analysisReady==true
        ...
        correlationErrors=decision.correlationErrors,
        exactYawMatchQuality=decision.exactYawMatchQuality,
        pitchControlQuality=decision.pitchControlQuality,
        primaryHorizontalTouch=decision.primaryHorizontalTouch,
        primaryHorizontalRelay=decision.primaryHorizontalRelay,
        primaryVerticalTouch=decision.primaryVerticalTouch,
        primaryVerticalRelay=decision.primaryVerticalRelay,
        headHorizontalTouch=decision.headHorizontalTouch,
        headHorizontalRelay=decision.headHorizontalRelay,
        headVerticalTouch=decision.headVerticalTouch,
        headVerticalRelay=decision.headVerticalRelay,
        primaryDifferenceAfterYawMatch=decision.primaryDifferenceAfterYawMatch,
        headDifferenceAfterYawMatch=decision.headDifferenceAfterYawMatch,
        relativeHeadRigGeometryDifference=decision.relativeHeadRigGeometryDifference,
        pitchExplainsAsymmetry=decision.pitchExplainsAsymmetry,
        rigGeometryExplainsAsymmetry=decision.rigGeometryExplainsAsymmetry,
        statisticalMethod=decision.statisticalMethod,
        statisticalUncertainty=decision.statisticalUncertainty,
        descriptiveDifference=decision.descriptiveDifference,
        ...
        externalPCReferenceUsed=decision.externalPCReferenceUsed,
        externalPCReferenceWindow=decision.externalPCReferenceWindow,
        pcVisualAnchorCandidate=decision.pcVisualAnchorCandidate,
        headScreenSpaceStability=decision.headScreenSpaceStability,
        primaryScreenSpaceStability=decision.primaryScreenSpaceStability,
        subjectScreenSpaceStability=decision.subjectScreenSpaceStability,
...
local function sampleReportLine(sample)
    local parts={
        "frame="..tostring(sample.frame),"phase="..tostring(sample.phase),
...
getgenv().PCV613Report=function(includeEvidence)
    local diagnostics=getgenv().PCV613Diagnostics()
    local lines={"=== PC MOVEMENT V613 REPORT ==="}
    ...
    lines[#lines+1]=""
    lines[#lines+1]="=== V613 REQUIRED AXIS/PITCH/RIG DECISION ==="
    ...
    lines[#lines+1]="correlationErrors = "..tostring(diagnostics.correlationErrors)
    lines[#lines+1]="exactYawMatchQuality = "..tostring(diagnostics.exactYawMatchQuality)
    lines[#lines+1]="pitchControlQuality = "..tostring(diagnostics.pitchControlQuality)
    lines[#lines+1]="primaryHorizontalTouch = "..tostring(diagnostics.primaryHorizontalTouch)
    lines[#lines+1]="primaryHorizontalRelay = "..tostring(diagnostics.primaryHorizontalRelay)
    lines[#lines+1]="primaryVerticalTouch = "..tostring(diagnostics.primaryVerticalTouch)
    lines[#lines+1]="primaryVerticalRelay = "..tostring(diagnostics.primaryVerticalRelay)
    lines[#lines+1]="headHorizontalTouch = "..tostring(diagnostics.headHorizontalTouch)
    lines[#lines+1]="headHorizontalRelay = "..tostring(diagnostics.headHorizontalRelay)
    lines[#lines+1]="headVerticalTouch = "..tostring(diagnostics.headVerticalTouch)
    lines[#lines+1]="headVerticalRelay = "..tostring(diagnostics.headVerticalRelay)
    lines[#lines+1]="primaryDifferenceAfterYawMatch = "..tostring(diagnostics.primaryDifferenceAfterYawMatch)
    lines[#lines+1]="headDifferenceAfterYawMatch = "..tostring(diagnostics.headDifferenceAfterYawMatch)
    lines[#lines+1]="relativeHeadRigGeometryDifference = "..tostring(diagnostics.relativeHeadRigGeometryDifference)
    lines[#lines+1]="pitchExplainsAsymmetry = "..tostring(diagnostics.pitchExplainsAsymmetry)
    lines[#lines+1]="rigGeometryExplainsAsymmetry = "..tostring(diagnostics.rigGeometryExplainsAsymmetry)
    lines[#lines+1]="statisticalMethod = "..tostring(diagnostics.statisticalMethod)
    lines[#lines+1]="statisticalUncertainty = "..tostring(diagnostics.statisticalUncertainty)
    lines[#lines+1]="descriptiveDifference = "..tostring(diagnostics.descriptiveDifference)
    ...
    lines[#lines+1]="pcVisualAnchorCandidate = "..tostring(diagnostics.pcVisualAnchorCandidate)
    lines[#lines+1]="headScreenSpaceStability = "..tostring(diagnostics.headScreenSpaceStability)
    lines[#lines+1]="primaryScreenSpaceStability = "..tostring(diagnostics.primaryScreenSpaceStability)
    lines[#lines+1]="subjectScreenSpaceStability = "..tostring(diagnostics.subjectScreenSpaceStability)
...
refreshLiveStatus=function()
    ...
    local rejectedA=a.rejectedMovingFrames+a.rejectedWrongRouteFrames+a.rejectedLowYawFrames+a.rejectedProjectionFrames
    local rejectedB=b.rejectedMovingFrames+b.rejectedWrongRouteFrames+b.rejectedLowYawFrames+b.rejectedProjectionFrames
    statusLabel.Text=string.format("fase=%s • %s • relay=%s • corrErr=%d\nA válidas=%d rejeitadas=%d | B válidas=%d rejeitadas=%d",
        currentPhase,stateText,relayText,frameCorrelationErrors,a.validStandingFrames,rejectedA,
        b.validStandingFrames,rejectedB)
...
local function createPanel()
    ...
    title.Text="V613 • STANDING PAREADO"; title.TextColor3=Color3.fromRGB(103,232,249)
    ...
    instructions.Text="1) INICIAR → FASE A; fique parado e só gire ~20 s\n2) Toque FASE B; continue parado e repita ~20 s\n3) Aguarde COLETA ATIVA em cada fase\n4) PARAR → COPIAR antes de sair"
    ...
    uiConnections[#uiConnections+1]=copy.Activated:Connect(function()
        stopProbe()
        local ok=copyToClipboard(getgenv().PCV613Report(true))
...
getgenv().PCV613Start=startProbe
getgenv().PCV613PhaseA=function() return beginControlledPhase("touch") end
getgenv().PCV613PhaseB=function() return beginControlledPhase("relay") end
getgenv().PCV613Stop=function() stopProbe(); return getgenv().PCV613Report(false) end
getgenv().PCV613EmergencyV500=function()
    stopProbe()
    if type(baseSetRelay)=="function" then pcall(baseSetRelay,false) end
    refreshPhaseButtons()
...
getgenv().__PCMobileAimCleanup=function()
    ...
    screenGui=nil
    restoreHooks()
    getgenv().PCV613Start=nil
    getgenv().PCV613PhaseA=nil
    getgenv().PCV613PhaseB=nil
    getgenv().PCV613Stop=nil
    getgenv().PCV613EmergencyV500=nil
    getgenv().PCV613Diagnostics=nil
    getgenv().PCV613Report=nil
    if baseCleanup then pcall(baseCleanup) end
    getgenv().__PCMobileAimCleanup=nil
end

addEvidence("READY","V613 axis-pitch-rig Touch-vs-relay control ready; observational geometry only")
warn("[V613] axis-pitch-rig geometry control ready | use mobile panel")
